...
if args.data == 'tp53':
    # Load data
    x, l, m = io.load_training_rama('data/TP53', postfix='_10ns_20ns')
    ...
    x_full, _, m_full = io.load_training_rama('data/TP53', postfix='')

    # Get x_train_full
    m_train_idx = [m_full.index(m) for m in m_train]
    x_train_full = x_full[m_train_idx]
    m_test_idx = [m_full.index(m) for m in m_test]
    x_test_full = x_full[m_test_idx]
    assert(x_train_full.shape == xtrs)
    assert(x_test_full.shape == xtes)
    #TODO reshape it?! how to match time points...
