...
parser.add_argument('-m', '--method', type=str, choices=['dnn', 'cnn'],
                    default='cnn', help='Method for building the classifier')
...
if args.test:
    ...
    train_densities, test_densities, train_labels, test_labels = \
        train_test_split(train_densities, train_labels, test_size=0.15,
...
scaletransform.fit(np.asarray(train_densities))
train_densities = scaletransform.transform(train_densities)
if args.test:
    test_densities = scaletransform.transform(test_densities)
#'''
...
train_densities = logtransform.transform(train_densities)
if args.test:
    test_densities = logtransform.transform(test_densities)
#"""
...
# Try
#for d, l in zip(train_densities, train_labels):
#    print(nn.tf.nn.softmax(trained_nn_model(d.reshape(1, -1))), l)
#for d, l in zip(test_densities, test_labels):
#    print(nn.tf.nn.softmax(trained_nn_model(d.reshape(1, -1))), l)
#    plt.figure()
